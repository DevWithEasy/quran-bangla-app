...
class DbService {
  ...
  static async getAyah(surahId, ayahNumber) {
    try {
      return await Database.getAyahs(surahId, ayahNumber);
    } catch (error) {
      console.error(`Error getting ayahs for surah ${surahId}:`, error);
      throw error;
    }
  ...
  static async searchAyahs(query) {
    try {
      return await Database.searchAyahs(query);
    } catch (error) {
  ...
  static async getSuraUrl(suraId, reciterId) {
    try {
      return await Database.getSuraUrlFast(suraId, reciterId);
    } catch (error) {
      console.error(
        `Error getting audio for surah ${suraId}, reciter ${reciterId}:`,
        error,
      );
      throw error;
    }
  ...
  static async getSurahTimestamps(reciterId, suraId) {
    try {
      return await Database.getSurahTimestamps(reciterId, suraId);
    } catch (error) {
      console.error(
        `Error getting timestamps for reciter ${reciterId}, surah ${suraId}:`,
        error,
      );
      throw error;
    }
