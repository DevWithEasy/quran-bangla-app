...
export default function DownloadScreen() {
  ...
  const checkExistingData = async () => {
    try {
      ...
      const requiredFiles = [
        `${DATA_PATH}/reciters.json`,
        `${DATA_PATH}/surahs.json`,
        `${DATA_PATH}/verses`,
      ];
  ...
  const unzipAppData = async (zipPath) => {
    try {
      ...
      throw new Error("ফাইল আনজিপ করতে সমস্যা হয়েছে");
    }
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setCurrentStep("পুরাতন ডেটা পরিষ্কার করা হচ্ছে...");
      ...
      await unzipAppData(uri);

      setCurrentStep("সেটআপ সম্পন্ন হচ্ছে...");
      await AsyncStorage.setItem("reciter", "4");
      await AsyncStorage.setItem("data_version", "1.0.0");

      await FileSystem.deleteAsync(ZIP_PATH, { idempotent: true });

      router.replace("/quran");
    } catch (error) {
      console.error("Download or setup failed:", error);
      Alert.alert(
        "ত্রুটি",
        dataCorrupted 
          ? "ডাটাবেস ফাইল ক্ষতিগ্রস্ত হয়েছে, নতুন করে ডাউনলোড করা হচ্ছে"
          : "ডাউনলোড বা সেটআপে সমস্যা হয়েছে",
        [
          {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        if (!hasData || dataCorrupted) {
          setShowDownloadModal(true);
          if (dataCorrupted) {
            Alert.alert(
              "ডাটাবেস সমস্যা",
              "ডাটাবেস ফাইলগুলো ক্ষতিগ্রস্ত হয়েছে, নতুন করে ডাউনলোড করা প্রয়োজন",
              [
                {
                  text: "ডাউনলোড করুন",
        ...
        <View style={styles.modalBackground}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              {dataCorrupted ? "ডাটাবেজ রিপেয়ার" : "ডাটাবেজ ফাইল ডাউনলোড"}
            </Text>
            <Text style={styles.modalText}>
              {dataCorrupted
                ? "ডাটাবেস ফাইল ক্ষতিগ্রস্ত হয়েছে, নতুন করে ডাউনলোড করতে হবে"
            ...
            <View style={styles.buttonGroup}>
              ...
              >
                <Text style={styles.buttonText}>
                  {dataCorrupted ? "পুনরায় ডাউনলোড করুন" : "ডাউনলোড করুন"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
